...
namespace CrossUI.SDL2
{
    ...
    {
        private static readonly NativeLibrary SDL2Lib = LoadSDL();
        private static NativeLibrary LoadSDL()
        {
            string[] names;
            ...
            //TODO fix
            var lib = new NativeLibrary("SDL2.dll");
            return lib;
        }
        ...
        public static T LoadFunction<T>(string name)
        {
            try
            {
                return SDL2Lib.LoadFunction<T>(name);
            }
            catch
            {
                Debug.WriteLine(
